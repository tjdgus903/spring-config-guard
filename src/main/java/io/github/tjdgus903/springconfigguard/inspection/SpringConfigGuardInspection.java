...
public final class SpringConfigGuardInspection extends LocalInspectionTool {
    ...
    private void registerFindings(PsiFile file, ConfigProfile profile, ProblemsHolder holder) {
        ...
        for (ConfigEntry entry : entries) for (Finding finding : ruleEngine.analyze(entry, context)) {
            ...
            holder.registerProblem(target, formatMessage(finding), ProblemHighlightType.GENERIC_ERROR_OR_WARNING);
        }
    }
    private PsiElement locateTarget(PsiFile file, ConfigEntry entry) {
        Document document = PsiDocumentManager.getInstance(file.getProject()).getDocument(file);
        if (document == null || document.getLineCount() == 0) return file;
        ...
        if (entry.value() != null && !entry.value().isEmpty()) {
            ...
            if (valueOffset >= 0) absoluteOffset += valueOffset;
        }
        if (absoluteOffset >= file.getTextLength() && file.getTextLength() > 0) absoluteOffset = file.getTextLength() - 1;
