...
플러그인 최소 빌드 번호는 253(IntelliJ IDEA 2025.3)입니다. Plugin Verifier는 최소 지원 버전인
2025.3.3과 현재 개발 기준인 2026.1.3을 모두 검사합니다. 실제 IDE 화면 테스트는 현재 개발 기준에서
수행하며, 그 밖의 제품·버전 조합의 실행 호환성을 이번 CI만으로 확인했다고 간주하지 않습니다.
...
- 디스크에는 안전한 값을 둔 미추적 운영 설정 문서를 편집기에서만 위험한 값으로 바꾸고,
  수동 변경 설정 보고서가 저장 전 캐시 문서의 SCG008을 포함하는지 확인합니다.
- 추적 중인 운영 설정 문서에도 `server.error.include-message`를 저장하지 않고 추가한 뒤,
  같은 보고서가 이 편집기 내용의 SCG006을 함께 포함하는지 확인합니다.
- 실제 커밋 흐름은 추적 문서를 먼저 저장하므로 사전검사가 revision 기반 6개 finding을
