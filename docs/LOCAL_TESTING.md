...
- 추적 중인 운영 설정 문서에 `server.error.include-message`를 저장하지 않고 추가한 뒤,
  수동 변경 설정 보고서가 이 편집기 내용의 SCG006을 포함하는지 확인합니다.
- 같은 문서를 선택한 실제 커밋 사전검사는 저장 전 편집기 내용과 별도 미추적 파일을 포함하지 않고
  revision 기반 5개 finding만 집계하며, 경고 후 커밋을 막지 않는지 확인합니다.
